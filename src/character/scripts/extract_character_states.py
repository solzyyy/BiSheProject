...
从事件图谱中提取人物内在状态变化（target_type="character"）。
通过 mentions.json 找到人物经历的事件，提取人物内在状态变化。
"""
...
from core.prompt_overrides import SLOT_CHARACTER_STATE_EXTRACT, read_prompt_with_override


# 提示词文件路径
CHARACTER_STATE_EXTRACT_PROMPT_PATH = (
    Path(__file__).resolve().parent.parent.parent.parent / "prompts" / "character_state_extract_prompt.txt"
)


def load_character_state_extract_template() -> str:
    """加载人物内在状态提取提示词模板 📄（支持环境变量覆盖）"""
    return read_prompt_with_override(CHARACTER_STATE_EXTRACT_PROMPT_PATH, SLOT_CHARACTER_STATE_EXTRACT)
...
def build_prompt(
    ...
    # 构建 mentions 信息（mentions_info 已经过滤过，包含该实体的所有 mentions，包括别名）
    mentions_text = "\n".join([
        f"- {m.get('name', '未知')}: {m.get('action_hint', '')} | {m.get('context_snippet', '')[:100]}"
    ...
    return template.format(
        ...
        character_id=character_id,  # 实体ID（如：C010）
        character_name=character_name,  # 规范名称（如：王佛）
...
async def extract_character_states_from_event(
    ...
    Args:
        ...
        character_name: 人物名称
        character_id: 人物ID
        mentions_info: 该事件中该人物的 mentions 信息
        
    Returns:
        状态变化列表
    """
    prompt = build_prompt(template, event, character_name, character_id, mentions_info)
    ...
    try:
        ...
        for sc_data in state_changes_data:
            try:
                ...
                # 确保 target_type 为 "character"
                sc_data["target_type"] = "character"
                
                # 处理 LLM 可能返回的 "id" 字段（应该使用 "target_id"）
                if "id" in sc_data and "target_id" not in sc_data:
                    # 如果 LLM 返回了 "id" 而不是 "target_id"，将其转换为 "target_id"
                    sc_data["target_id"] = sc_data.pop("id")
                
                # 确保 target_id 正确（强制使用传入的 character_id，忽略 LLM 返回的）
                sc_data["target_id"] = character_id
                
                # 验证 target_id 格式（应该是 C001, C002 等格式）
                if not isinstance(sc_data["target_id"], str) or not sc_data["target_id"].startswith("C"):
                    print(f"[yellow]⚠️  警告：target_id 格式异常: {sc_data['target_id']}，已修正为: {character_id}[/yellow]")
                    sc_data["target_id"] = character_id
                ...
                # 验证：检查 logic_impact 是否真的描述的是该人物的状态变化
                # 如果 logic_impact 明确描述了其他人物（而不是该人物）的行为，可能是 LLM 提取错误
                logic_impact = sc_data.get("logic_impact", "")
                value = sc_data.get("value", "")
                
                # 检查 logic_impact 中是否明确描述了其他人物（而不是该人物）的行为
                # 例如：如果提取"卫士"的状态变化，但 logic_impact 说"林采取了...行为"，这是错误的
                # 这里使用简单的启发式检查：如果 logic_impact 以其他人物名称开头，可能是错误的
                # 但不过滤，因为可能是正确的（例如："被卫士控制"描述的是"林"的状态，但提到了"卫士"）
                # 所以这里只做警告，不强制过滤
        ...
        event_id = event.get("event_id", "未知")
        print(f"[red]❌ 提取 {event_id} 中 {character_name} 的状态失败: JSON 解析错误[/red]")
        print(f"[yellow]错误详情: {e}[/yellow]")
        return []
    except Exception as e:
        event_id = event.get("event_id", "未知")
        print(f"[red]❌ 提取 {event_id} 中 {character_name} 的状态失败: {e}[/red]")
        import traceback
...
async def main_async():
    ...
    default_output = Path("out/character_states.json")
    ...
    template = load_character_state_extract_template()
    print("[green]✓[/green] 提示词模板加载成功")
    print()
    
    # 按实体组织：找出每个实体经历的所有事件
    # 1. 先找出所有实体（使用规范名称）
    print(f"[cyan]📋 找出所有实体...[/cyan]")
    all_entity_names = list(entities.keys())
    print(f"[green]✓[/green] 找到 {len(all_entity_names)} 个实体")
    print()
    
    # 2. 根据 mentions 找到每个实体经历的事件（处理别名匹配）
    print(f"[cyan]🔍 根据 mentions 查找每个实体经历的事件（处理别名匹配）...[/cyan]")
    entity_events_map = defaultdict(set)  # 规范名称 -> 事件ID集合
    event_mentions_map = defaultdict(list)  # 事件ID -> mentions列表
    ...
    for mention in mentions:
        ...
        if not event_id or not mention_name:
            continue
        ...
        if canonical_name and canonical_name in entities:
            # 使用规范名称作为key
            entity_events_map[canonical_name].add(event_id)
            event_mentions_map[event_id].append(mention)
            # 记录mention中的name到规范名称的映射
            mention_name_to_canonical[mention_name] = canonical_name
    
    # 统计信息
    total_entity_events = sum(len(event_set) for event_set in entity_events_map.values())
    print(f"[green]✓[/green] 找到 {total_entity_events} 个实体-事件对")
    print()
    
    # 批量提取（带进度条和并发控制）
    print(f"[cyan]🚀 开始处理实体经历的事件（并发处理）...[/cyan]")
    print()
    
    # 创建 Semaphore 控制并发数（最多同时处理 5 个任务）
    semaphore = asyncio.Semaphore(5)
    
    # 准备所有任务：遍历实体，对每个实体经历的事件提取状态
    all_tasks = []
    for canonical_name in all_entity_names:
        if canonical_name not in entity_events_map:
            continue
        ...
        if not character_id:
            print(f"[yellow]⚠️  实体 {canonical_name} 没有ID，跳过[/yellow]")
            continue
        
        # 获取该实体经历的所有事件（按事件ID排序）
        entity_event_ids = sorted(entity_events_map[canonical_name], key=lambda eid: (
            int(eid[1:]) if eid.startswith('E') and eid[1:].isdigit() else 0
        ))
        ...
        for event_id in entity_event_ids:
            ...
            # 获取该事件中该实体的所有mentions（包括别名）
            # 需要匹配规范名称或任何别名
            mentions_info = []
            for m in event_mentions_map.get(event_id, []):
                mention_name = m.get("name")
                # 检查mention中的name是否映射到这个规范名称
                if mention_name_to_canonical.get(mention_name) == canonical_name:
                    mentions_info.append(m)
            
            all_tasks.append((event_id, event, canonical_name, character_id, mentions_info))
    ...
    async def process_task_with_semaphore(task_data, progress, task) -> tuple:
        ...
        async with semaphore:
            event_id, event, char_name, character_id, mentions_info = task_data
            progress.update(task, description=f"处理: {event_id} - {char_name}...")
            
            state_changes = await extract_character_states_from_event(
                llm_client, template, event, char_name, character_id, mentions_info
            )
            
            progress.advance(task)
            
            failed = not state_changes
            return state_changes, f"{event_id}-{char_name}", failed
    ...
    with Progress(
        ...
        task = progress.add_task("提取人物状态...", total=total_tasks)
    ...
    print(f"\n[green]🎉 处理完成！共提取 {len(all_state_changes)} 个人物内在状态变化[/green]")
    print()
    
    # 统计信息
    unique_characters = set(sc.target_id for sc in all_state_changes)
    ...
    print(f"  • 涉及人物数: {len(unique_characters)}")
    ...
    print(f"[green]✓[/green] 人物内在状态已保存到: {output_path}")
